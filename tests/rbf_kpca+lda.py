...
logger, plotter = helpers.Logger(filename='seizure_detection_rbf_kpca+lda'), helpers.Plotter()
...
def preprocess(x_train, y_train, x_test):
    ...
    # Plot pca pov vs k.
    plotter.pov_analysis(pca.explained_var, subfolder='pca_analysis/all_components-1', filename='rbf')

    logger.log('\tApplying Principal Component Analysis with params:')
    pca = KPCA(Kernels.RBF, sigma=4.5, n_components=0.9)
    logger.log('\t' + str(pca.get_params()))
