...
def preprocess(x_train, y_train, x_test):
    ...
    pca = KPCA(Kernels.RBF, sigma=3.1)
    logger.log('\t' + str(pca.get_params()))
    x_train = pca.fit_transform(x_train)

    # Plot pca pov vs k.
    plotter.pov_analysis(pca.explained_var, subfolder='pca_analysis/all_components-1', filename='rbf-bad')

    logger.log('\tApplying Principal Component Analysis with params:')
    pca = KPCA(Kernels.RBF, sigma=3.1, n_components=0.9)
    logger.log('\t' + str(pca.get_params()))
    x_train = pca.fit_transform(x_train)

    # Plot pca pov vs k.
    plotter.pov_analysis(pca.explained_var, subfolder='pca_analysis/pov_0.9', filename='rbf-bad')

    plotter.scatter_pcs(pca.alphas[:, :3], y_train, class_labels=helpers.datasets.get_eeg_name,
                        subfolder='scatters/pca/rbf-bad', filename='pov_0.9_3pcs')
    plotter.scatter_pcs(pca.alphas[:, :2], y_train, class_labels=helpers.datasets.get_eeg_name,
                        subfolder='scatters/pca/rbf-bad', filename='pov_0.9_2pcs')
    plotter.scatter_pcs(pca.alphas[:, 0], y_train, class_labels=helpers.datasets.get_eeg_name,
                        subfolder='scatters/pca/rbf-bad', filename='pov_0.9_1pc')
